...
#define POSHOLD_CONTROLLER_TO_PILOT_MIX_TIMER   (50*2)  // Set it from 100 to 200, the number of centiseconds loiter and manual commands are mixed to make a smooth transition.
...
float proportion_a = 0;
float integral_a = 0;
float derivative_a = 0;

int roll_avoid = 0;
int anlge_bypass_counter = 0;
int stay_in_pilot_mode = 0;
...
void Copter::poshold_run()
{
    ...
    if (ap.land_complete) {
        ...
        debug_counter_1 = target_roll/100;
        //zx get radar flag
        //if inside warning zone, set warning zone, speed and anlge limit.

        if (Est_Target_Distance < Warning_Zone_fense && Est_Target_Distance > Red_Zone_fense)
        {
            Warning_Zone_flag = 1;
            if (Est_Target_Distance > Red_Zone_fense_buffer)
            {
                waring_zone_speed_limit = (Est_Target_Distance - Red_Zone_fense_buffer)/(Warning_Zone_fense - Red_Zone_fense_buffer)*waring_zone_speed_max;
                waring_zone_angle_limit = waring_zone_angle_max - ((Est_Target_Distance - Red_Zone_fense_buffer)/(Warning_Zone_fense - Red_Zone_fense_buffer)*waring_zone_angle_max);
            }
            else
            {
                waring_zone_speed_limit = 0;
                waring_zone_angle_limit = waring_zone_angle_max;
            }
            if (target_pitch < 0 && (Est_Target_Angle < 15 && Est_Target_Angle > -15))
            {
                stay_in_pilot_mode = 1;
                roll_avoid = 1;
                if (anlge_bypass_counter < 200)
                {
                    anlge_bypass_counter++;
                }
                else if (anlge_bypass_counter < 400)
                {
                    anlge_bypass_counter++;
                    roll_avoid = 0;
                }
                ...
                {
                    roll_avoid = 0;
                    anlge_bypass_counter = 0;
                }
            }
            else
            {
                stay_in_pilot_mode = 0;
                roll_avoid = 0;
                anlge_bypass_counter = 0;
            }
        }
        ...
        { 
            Warning_Zone_flag = 0;
            waring_zone_speed_limit = waring_zone_speed_max;
            waring_zone_angle_limit = 0;
            anlge_bypass_counter = 0;
            stay_in_pilot_mode = 0;
            roll_avoid = 0;
        }

        debug_counter_3 = anlge_bypass_counter;

        // if inside red zone set red zone, else clear the red zone
        if (Est_Target_Distance < Red_Zone_fense && Est_Target_Angle < 30 && Est_Target_Angle > -30)
        {
            Red_Zone_flag = 1;          
        }
        else
        ...
        switch (poshold.roll_mode) {
            ...
            case POSHOLD_PILOT_OVERRIDE:
                ...
                if (is_zero(target_roll) && (fabsf(poshold.pilot_roll) < 2 * g.poshold_brake_rate) && stay_in_pilot_mode == 0) {
                ...
                //if in warning zone, apply roll input angle, (by applying pid control loop)
                if (roll_avoid == 1 && vel_right < 80 && vel_right > -80 && Est_Target_Distance < 7)
                {               
                    if ((Est_Target_Angle > (0 -waring_zone_angle_limit)) && (Est_Target_Angle < 0) && (target_pitch <0))
                    {
                        waring_zone_angle_limit = 0 - waring_zone_angle_limit;
                        debug_counter_5 = waring_zone_angle_limit;
                        poshold.pilot_roll = PID_Control_WarningZone_a(Est_Target_Angle, waring_zone_angle_limit, target_roll, Kp_a, Ki_a, Kd_a);
                        // update the pilot lean angle to the pid filtered angle.
                        debug_counter_4 = poshold.pilot_roll;
                    }
                    else if ((Est_Target_Angle < waring_zone_angle_limit) && (Est_Target_Angle > 0) && (target_pitch <0))
                    {
                        poshold.pilot_roll = PID_Control_WarningZone_a(Est_Target_Angle, waring_zone_angle_limit, target_roll, Kp_a, Ki_a, Kd_a);
                        // update the pilot lean angle to the pid filtered angle.
                        debug_counter_4 = poshold.pilot_roll;
                    ...
                    {
                        // initialise warining zone pid loop parameters
                        error_a = 0;
                        errorT_a = 0;
                        last_error_a = 0;
                        proportion_a = 0;
        ...
        switch (poshold.pitch_mode) {
            ...
            case POSHOLD_PILOT_OVERRIDE:
                ...
                if ((Red_Zone_flag == 1 && vel_fw > POSHOLD_SPEED_0) || (Red_Zone_flag == 1 && poshold.pilot_pitch < 0))//zx
                {
                    // initialise BRAKE mode
                    poshold.pitch_mode = POSHOLD_BRAKE;       // set brake pitch mode
                    poshold.brake_pitch = 0;                 // initialise braking angle to zero
                ...
                //if in warning zone, limit the pitch input angle, speed(by applying pid control loop)
                if (Warning_Zone_flag == 1)
                {                        
                    if (vel_fw > waring_zone_speed_limit)
                    {
                        poshold.pilot_pitch = PID_Control_WarningZone(vel_fw, waring_zone_speed_limit, poshold.pilot_pitch, Kp, Ki, Kd);
                        // update the pilot lean angle to the pid filtered angle.
                        float temp = 0-poshold.pilot_pitch;
                        poshold_update_pilot_lean_angle(temp , poshold.pilot_pitch);
                    }
                    else
                    {
                        // initialise warining zone pid loop parameters
                        error = 0;
                        errorT = 0;
                        last_error = vel_fw - waring_zone_speed_limit;
                        proportion = 0;
                        integral = 0;
                        derivative = 0;
                    } 
            ...
            case POSHOLD_BRAKE_READY_TO_LOITER:
                ...
                // check for pilot input //zx
                if ((!is_zero(target_pitch) && Red_Zone_flag == 0 ) || (target_pitch > 0)) {
                    // init transition to pilot override
...
float Copter::PID_Control_WarningZone(float current_state, float target_state, float input, float kp, float ki, float kd)
{
    ...
    last_error = error;
    float temp = input + proportion + integral + derivative;
    if (temp > aparm.angle_max)
    {
        temp = aparm.angle_max;
    }
    if (temp < -aparm.angle_max)
    {
        temp = -aparm.angle_max;
...
float Copter::PID_Control_WarningZone_a(float current_state, float target_state, float input, float kp, float ki, float kd)
{
    error_a = current_state - target_state;

    proportion_a = error_a * kp;
    integral_a = errorT_a * ki;
    derivative_a = (error_a - last_error_a) * kd;

    last_error_a = error_a;
    float temp = proportion_a + integral_a + derivative_a;
    if (temp > 700)
    {
        temp = 700;
    }
    ...
    float temp1 = input + temp;
    if (temp1 > aparm.angle_max)
    {
        temp1 = aparm.angle_max;
    }
    if (temp1 < -aparm.angle_max)
    {
        temp1 = -aparm.angle_max;
