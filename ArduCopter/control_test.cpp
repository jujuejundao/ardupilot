//Modified from loiter mode

#include "Copter.h"

//Init and run calls for loiter flight mode
float Red_Zone = 2;
float Warning_Zone = 8;
float Target_roll;
float roll_velocity;
...
void Copter::test_run()
{
    ...
    case Loiter_Flying:
        ...
        // Check for red zone, if enters the red zone, clear user input and stop immediately
        if (Est_Target_Distance < Red_Zone)
        {
            wp_nav->clear_pilot_desired_acceleration();
            attitude_control->input_euler_angle_roll_pitch_euler_rate_yaw(wp_nav->get_roll(), wp_nav->get_pitch(), target_yaw_rate, get_smoothing_gain());
        }
        else if (Est_Target_Distance < Warning_Zone)
        {
            if (roll_velocity > target_velocity)
            {
                attitude_control->input_euler_angle_roll_pitch_euler_rate_yaw(Target_roll, wp_nav->get_pitch(), target_yaw_rate, get_smoothing_gain());
            }  
        }
        else
        {
