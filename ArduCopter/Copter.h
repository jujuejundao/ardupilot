...
//radar main code parameters definition.
...
AP_HAL::UARTDriver *uart_T = nullptr;
uint8_t No_of_targets;
uint8_t Rollcount;
uint8_t Target_index;
float Min_Target_Reflected_Area = 0;
float Min_Target_Distance = 100;
float Min_Target_Speed = 0;
float Min_Target_Angle = 0;
float Min_Target_SNR = 0;   
float debug_counter_1=0;
float debug_counter_2=0;
float debug_counter_3=0;
float debug_counter_4=0;
float debug_counter_5=0;
float range_list[10];
uint8_t range_key[10];

bool process_input_radar();
bool radar_estimator(float, float, float);
float PID_Control_WarningZone(float current_state, float target_state, float input, float kp, float ki, float kd);
float PID_Control_WarningZone_a(float current_state, float target_state, float input, float kp, float ki, float kd);
...
float Red_Zone_fense = 2.5;
float Red_Zone_fense_buffer = 3;
float Warning_Zone_fense = 10;
float waring_zone_speed_limit;
float waring_zone_speed_max = 200; // in cm/s
float waring_zone_angle_limit;
float waring_zone_angle_max = 40; //in degrees
...
float data_abord = 0;
    ...
    void test_run();
    bool radar_init(bool ignore_checks);
    void radar_run();
